    ...
    <main class="page-main">
      <div class="container">
        ...
        <section class="request-summary" aria-label="요청 현황">
          <button :class="{ active: statusFilter === 'PENDING' }" @click="toggleStatus('PENDING')">
            <span class="summary-icon pending" aria-hidden="true">○</span>
            <div><small>검토 대기</small><strong>{{ pendingCount }}</strong></div>
          </button>
          <button :class="{ active: statusFilter === 'ACTIVE' }" @click="toggleStatus('ACTIVE')">
            <span class="summary-icon active" aria-hidden="true">↔</span>
            <div><small>대여 중</small><strong>{{ activeCount }}</strong></div>
          </button>
          <button :class="{ active: statusFilter === 'RETURN_REQUESTED' }" @click="toggleStatus('RETURN_REQUESTED')">
            <span class="summary-icon return" aria-hidden="true">✓</span>
            <div><small>반납 확인 대기</small><strong>{{ returnCount }}</strong></div>
          </button>
          <button :class="{ active: statusFilter === 'OVERDUE' }" @click="toggleStatus('OVERDUE')">
            <span class="summary-icon overdue" aria-hidden="true">!</span>
