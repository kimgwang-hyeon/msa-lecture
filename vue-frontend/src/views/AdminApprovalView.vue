      ...
      <div class="container">
        <template v-if="isManager">
          ...
          <section class="admin-summary" aria-label="처리 대기 현황">
            <button :class="{ active: tab === 'loan' }" @click="setTab('loan')">
              <span class="summary-mark green">↔</span>
              <div><small>대여 승인</small><strong>{{ loans.length }}</strong></div>
            </button>
            <button :class="{ active: tab === 'return' }" @click="setTab('return')">
              <span class="summary-mark blue">✓</span>
              <div><small>반납 확인</small><strong>{{ returns.length }}</strong></div>
            </button>
            <button :class="{ active: tab === 'acquisition' }" @click="setTab('acquisition')">
              <span class="summary-mark amber">＋</span>
              <div><small>도입 필요성</small><strong>{{ acquisitions.length }}</strong></div>
            </button>
            <button :class="{ active: tab === 'intake' }" @click="setTab('intake')">
              <span class="summary-mark purple">▦</span>
          ...
          <div v-else-if="filteredItems.length" class="approval-list">
            ...
            >
              ...
              <section v-else-if="tab === 'return'" class="return-panel">
                <strong>반납 검수 체크리스트</strong>
                <p>실물 확인 후 세 항목을 모두 체크해야 재고를 복원할 수 있습니다.</p>
                <div class="return-checks">
                  <label><input v-model="returnChecks[item.id].condition" type="checkbox" /> 외관·작동 상태</label>
                  <label><input v-model="returnChecks[item.id].accessories" type="checkbox" /> 구성품·충전기</label>
                  <label><input v-model="returnChecks[item.id].data" type="checkbox" /> 사용자 데이터 정리</label>
                </div>
                <button
                  class="btn btn-primary btn-sm"
...
.return-checks input { accent-color: var(--color-primary); }
