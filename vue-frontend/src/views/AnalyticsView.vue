      ...
      <div class="container">
        <template v-if="isManager">
          <div class="page-heading">
            <div>
              <span class="eyebrow ai-eyebrow">✦ SCIKIT-LEARN DEMAND INTELLIGENCE</span>
          ...
          <template v-else-if="forecast">
            ...
            <div class="forecast-toolbar">
              <div class="risk-summary" role="group" aria-label="위험 단계 필터">
                <button :class="{ active: riskFilter === 'ALL' }" @click="riskFilter = 'ALL'">전체 {{ forecast.items.length }}</button>
                <button class="risk-high" :class="{ active: riskFilter === 'HIGH' }" @click="riskFilter = 'HIGH'">부족 {{ riskCount('HIGH') }}</button>
                <button class="risk-medium" :class="{ active: riskFilter === 'MEDIUM' }" @click="riskFilter = 'MEDIUM'">주의 {{ riskCount('MEDIUM') }}</button>
                <button class="risk-low" :class="{ active: riskFilter === 'LOW' }" @click="riskFilter = 'LOW'">안정 {{ riskCount('LOW') }}</button>
              </div>
            ...
            <div v-if="filteredItems.length" class="forecast-grid">
              <article v-for="item in filteredItems" :key="item.category" class="forecast-card surface">
                <div class="forecast-head">
                  <div>
                    <span class="category-symbol" aria-hidden="true">{{ categoryIcon(item.category) }}</span>
                ...
                <div class="chart" :aria-label="`${categoryLabel(item.category)} 주별 예상 요청`">
            ...
            <details class="model-details surface">
              ...
              <div class="model-detail-body">
                ...
                <table v-if="candidateRows.length" class="model-table">
...
.decision-brief {
  ...
  padding: 20px;
  color: #fff;
  background: linear-gradient(120deg, #102a43, #21483f);
  border: 0;
}
.brief-icon {
  width: 48px;
...
.decision-brief span { color: #8ce0c7; font-size: 9px; font-weight: 800; letter-spacing: .08em; }
.decision-brief h2 { margin-top: 3px; font-size: 17px; line-height: 1.45; }
.decision-brief p { margin-top: 3px; color: #bfd0d6; font-size: 10px; }
.decision-brief > small { color: #b8c6cc; font-size: 9px; white-space: nowrap; }
.executive-grid {
  display: grid;
...
.model-highlight { background: var(--color-primary-light); border-color: #c7e3da; }
...
.model-strip > div:last-child { border: 0; background: #eef9f5; }
...
.bar-track i {
  ...
  width: min(32px, 65%);
  min-height: 7px;
  background: linear-gradient(#55b89b, #0b6b57);
