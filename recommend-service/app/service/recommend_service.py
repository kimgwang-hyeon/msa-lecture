...
class RecommendService:
    ...
    async def get_alternatives(
        ...
        """신규 구매 전에 동일 카테고리의 보유 교보재를 추천한다."""
        alternatives = await course_client.get_recommend_courses(
            category=category,
            exclude_ids=[]
        )
        alternatives = [
            course for course in alternatives
            if course.itemType == "OWNED" and course.availableQuantity > 0
        ][:self.MAX_RECOMMEND_COUNT]

        message = (
            "대여 가능한 보유 교보재를 먼저 확인해 주세요"
            if alternatives
            else "현재 대여 가능한 동일 카테고리 교보재가 없습니다"
        )
        return AlternativeRecommendResponse(
            category=category,
